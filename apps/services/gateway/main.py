...
async def proxy_list_books():
    ...
    r = await _call(
        book_breaker, lambda: httpx.get(f"{BOOK_SERVICE_URL}/books", timeout=TIMEOUT)
    )
    if r is None:
...
async def proxy_get_book(book_id: int):
    ...
    r = await _call(
        book_breaker,
        lambda: httpx.get(f"{BOOK_SERVICE_URL}/books/{book_id}", timeout=TIMEOUT),
    )
    if r is None:
        return _service_unavailable("book-service")
    return Response(
...
async def proxy_create_book(request: Request):
    ...
    r = await _call(
        book_breaker,
        lambda: httpx.post(f"{BOOK_SERVICE_URL}/books", json=body, timeout=TIMEOUT),
    )
    if r is None:
        return _service_unavailable("book-service")
    return Response(
...
async def proxy_register(request: Request):
    ...
    r = await _call(
        auth_breaker,
        lambda: httpx.post(f"{AUTH_SERVICE_URL}/register", json=body, timeout=TIMEOUT),
    )
    if r is None:
...
async def proxy_login(request: Request):
    ...
    r = await _call(
        auth_breaker,
        lambda: httpx.post(
            f"{AUTH_SERVICE_URL}/login", data=dict(body), timeout=TIMEOUT
        ),
    )
    if r is None:
        return _service_unavailable("auth-service")
...
async def proxy_me(request: Request):
    ...
    r = await _call(
        auth_breaker,
        lambda: httpx.get(
            f"{AUTH_SERVICE_URL}/me", headers={"Authorization": token}, timeout=TIMEOUT
        ),
    )
    if r is None:
        return _service_unavailable("auth-service")
...
async def proxy_list_orders(request: Request):
    ...
    r = await _call(
        order_breaker,
        lambda: httpx.get(
            f"{ORDER_SERVICE_URL}/orders", headers=headers, timeout=TIMEOUT
        ),
    )
    if r is None:
...
async def proxy_create_order(request: Request):
    ...
    r = await _call(
        order_breaker,
        lambda: httpx.post(
            f"{ORDER_SERVICE_URL}/orders", json=body, headers=headers, timeout=TIMEOUT
        ),
    )
    if r is None:
        return _service_unavailable("order-service")
...
async def proxy_list_inventory():
    ...
    r = await _call(
        inventory_breaker,
        lambda: httpx.get(f"{INVENTORY_SERVICE_URL}/inventory", timeout=TIMEOUT),
    )
    if r is None:
...
async def proxy_get_inventory(book_id: int):
    ...
    r = await _call(
        inventory_breaker,
        lambda: httpx.get(
            f"{INVENTORY_SERVICE_URL}/inventory/{book_id}", timeout=TIMEOUT
        ),
    )
    if r is None:
        return _service_unavailable("inventory-service")
